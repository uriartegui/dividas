...
export default function CollectionRulesPage() {
  ...
  async function toggleRule(days: number) {
    ...
    if (rule) {
      ...
      await api.put(`/collection-rules/${rule.id}`, {
        ...rule,
        active: !rule.active,
      });
      ...
      const { data } = await api.post("/collection-rules", {
        days_after_due: days,
        message_template:
          DEFAULT_TEMPLATES[days as keyof typeof DEFAULT_TEMPLATES],
        active: true,
      });
  ...
  return (
    <div className="p-8">
      ...
      <p className="text-gray-400 mb-8">
        Configure mensagens automáticas por WhatsApp após o vencimento.
        Disparadas todo dia às 08h.
      </p>
      ...
      <div className="space-y-6">
        {PRESETS.map((days) => {
          ...
          return (
            <div
              key={days}
              className={`bg-gray-800 rounded-xl p-6 border ${active ? "border-green-500/50" : "border-gray-700"}`}
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-white font-semibold text-lg">D+{days}</h2>
                  <p className="text-gray-400 text-sm">
                    {days === 1 ? "1 dia" : `${days} dias`} após o vencimento
                  </p>
                </div>
                <button
      ...
      <div className="mt-8 bg-gray-800 rounded-xl p-4 border border-gray-700">
        ...
        <ul className="text-gray-400 text-sm space-y-1">
          <li>
            • Todo dia às <strong className="text-white">08:00</strong> o
            sistema verifica dívidas vencidas
          </li>
          <li>
            • Envia WhatsApp para devedores cujo vencimento foi há exatamente
            D+1, D+7 ou D+30 dias
          </li>
          <li>
            • Não envia duplicatas — máximo 1 mensagem por devedor por dia
          </li>
          <li>
            • Só envia para dívidas com status{" "}
            <strong className="text-white">Pendente</strong> ou{" "}
            <strong className="text-white">Em negociação</strong>
          </li>
        </ul>
      </div>
    </div>
